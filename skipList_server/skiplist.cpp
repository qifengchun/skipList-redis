...
bool SkipList<K,V>::insert_element(const K key,const V value)
{
    ...
    if(current !=nullptr && current->getKey() == key){
        std::cout<<"key:"<<key<<"has existed,could't insert..."<<std::endl;
        return 0;
    }
    ...
    if(current == nullptr||current->getKey()!=key){
        ...
        //if random level is greater that skip list's current level, initialize update value with pointer to header
...
bool SkipList<K,V>::delete_element(K key)
{
    //lock_guard<mutex> lock(mtx);
    //Node<K,V> *update [_max_level+1];
    Node<K,V> *update = new Node<K,V>[_max_level+];
...
int Skiplist<K, V>::get_random_level(){
    //该randomLevel方法会随机生成1～_max_level之间的数，且：
    //          1/2的概率返回1
    //          1/4的概率返回2
    //          1/8的概率返回3
    int k=1;
