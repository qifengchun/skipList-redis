...
//客户端输入对应的命令。服务器对命令进行解析，在自己的命令集合中
//进行查找并执行
...
void Server::workerThread(int fd){
    Client* c = _client.find(fd)->second;//找到当前线程所对应的客户端
...
void Server::processListen(int fd){
    ...
    socklen_t clilen = sizeof(client->client_addr);
    int connfd = accept(fd,(struct sockaddr*)&client->client_addr,&clilen);
    if (connfd<0){
        std::cout<<"accept error"<<endl;
        return;
    }
    client->sockfd = connfd;
    std::cout<<"Accepted client:"<<inet_ntoa((client->client_addr).sin_addr)<<":"
            <<ntohs((client->client_addr).sin_port)<<std::endl;
    //增加与服务端关联的客户端数量
...
void Server::processSet(const Command &C, Client *c)
{
    ...
    if(flag){
        //向客户端发送数据
        str = "suncessfully insert";
    }else{
        str = "fail, key has exited";
    }
...
void Server::processDel(const Command &C, Client *c)
{
    string key = C._arg[1];
    bool flag = true;
    delCommand(c,key,flag); //删除指定key的键值对
    if(flag){
